...
If the new SVT is stored in addition to the old SVT, it MUST be added to the array of SVT tokens in the existing "svt" Header Parameter value.
...
An SVT according to this profile SHALL contain one instance of the SignedData claims object which SHALL contain the following data:
...
- ref -- This parameter MUST hold one of the following thee possible values.
  ...
  3. A URI that explicitly identifies the detached signed data and/or its location. How this URI is associated with the signed data is outside the scope of this specification.
